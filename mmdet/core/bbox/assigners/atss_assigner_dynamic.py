...
from ..coder import delta_xywh_bbox_coder


@BBOX_ASSIGNERS.register_module()
class ATSSAssignerDynamic(BaseAssigner):
    ...
    def assign(self,
        ...
        Args:
            ...
            predicted_bboxes (Tensor): predict anchors, shape(n, 4),4 represent xyxy.
        ...
        # 这里修改为giou
        overlaps = self.iou_calculator(bboxes, gt_bboxes, mode='giou')
        ...
        # compute center distance between all bbox and predicted_bbox with gt
        gt_cx = (gt_bboxes[:, 0] + gt_bboxes[:, 2]) / 2.0
        gt_cy = (gt_bboxes[:, 1] + gt_bboxes[:, 3]) / 2.0
        gt_points = torch.stack((gt_cx, gt_cy), dim=1)

        # compute center distance between all predicted_bbox and predicted_bbox and gt
        bboxes_cx = (bboxes[:, 0] + bboxes[:, 2]) / 2.0
        bboxes_cy = (bboxes[:, 1] + bboxes[:, 3]) / 2.0
        bboxes_points = torch.stack((bboxes_cx, bboxes_cy), dim=1)

        # compute center distance between all predicted_bbox and gt
        distances = (bboxes_points[:, None, :] -
                     gt_points[None, :, :]).pow(2).sum(-1).sqrt()

        if (self.ignore_iof_thr > 0 and gt_bboxes_ignore is not None
                and gt_bboxes_ignore.numel() > 0 and bboxes.numel() > 0 and predicted_bboxes.numel() > 0):
        ...
        overlaps_inf = torch.full_like(ious,
                                       -INF).t().contiguous().view(-1)
        index = candidate_idxs.view(-1)[is_pos.view(-1)]
        overlaps_inf[index] = ious.t().contiguous().view(-1)[index]
